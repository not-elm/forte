...
skills/
  ...
  codex-engine/        # Shared Codex CLI execution contract for all codex-based skills (invocation, budget tiers, wait, read-back)
  ...
  rust-diagnostics/    # Compact Cargo diagnostics with deterministic extraction and optional local Qwen analysis
...
- **Codex-based skills** (codex-investigate, parallel-research, plan-review, spec-review, trio-brainstorming, plus board `-cx` members): Invocation mechanics are defined once in `codex-engine/REFERENCE.md` — detached `codex exec` in read-only mode, prompt via temp file + stdin, completion signalled by a sentinel file carrying the exit code, a 900s Monitor wait ceiling, and full-file read-back. Each SKILL.md owns only its prompt, its `{PREFIX}`, and the `{TIER}` (`light` / `standard` / `deep`) it declares per call site; it never sets a timeout or a model. Reasoning effort comes from the tier and the user's `~/.codex/config.toml` is treated as a ceiling — the contract lowers effort for cheap workloads and never raises it. The engine also appends the Execution Budget block to every prompt and defines the `usage_limit` failure class, which callers must degrade around rather than abort on. `parallel-research`, `spec-review`, and `plan-review` additionally dispatch a Claude Code Agent in parallel within a single message.
...
- **Local-runner skills** (rust-diagnostics: automatic; local-implement: opt-in and explicit): Route ordinary Cargo diagnostics and — only when a caller asks for it — local implementation of one planned task through short skills whose standard-library runners keep raw inputs outside caller context. The runners are terminal execution steps and are never wrapped recursively. `local-implement` additionally owns every side effect of an implementation attempt — the model only returns file contents as data — and never creates commits; because its run can outlast any foreground call, it is launched detached and waited on via a sentinel file.
