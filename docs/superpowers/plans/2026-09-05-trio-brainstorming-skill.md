...
- State file at `docs/brainstorms/{topic-id}/STATE.md`; Claude is the sole writer; ≤ 200 lines target; directory deleted after the spec is written.
...
**Files:**
- None created or modified (trial artifacts under `docs/brainstorms/` are deleted by the skill itself; any trial spec under `docs/superpowers/specs/` is gitignored and may be removed by hand)
...
- Every question shown to you carries `[Codex]`, `[Claude]`, or `[共同]`.
...
- `trio-cp1` launch, then the interview; `trio-cp2` launch appears only after pending questions reach zero or an Assumption is invalidated (State's Changes shows `invalidates A{n}`).
- The approaches presented include at least one tagged `[Codex]` or `[共同]`.
...
Start a session with codex hidden from PATH, e.g.:

```bash
PATH=$(echo "$PATH" | tr ':' '\n' | grep -v "$(dirname "$(command -v codex)")" | paste -sd: -) claude
```
