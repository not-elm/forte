...
- Invoke `forte:team-composer`: `topic` = bug description, `team_name` = kebab-case `{investigation-id}`.
...
- Error: L1 file not found → ❓; L2 Codex error → leader terminates the skill immediately. Clean up working directory and temp files (`rm -f /tmp/cx-*.txt /tmp/codex-*.txt`). **Exception:** a Codex usage limit leaves the round unaudited (L1 results stand) and triggers -cx Withdrawal instead of termination — see board-engine REFERENCE.md.
